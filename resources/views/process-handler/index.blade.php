...
@section('content')
    <div class="container">
        ...
        <a href="{{ route('user.toggle-position', 'LONG') }}"
            class="btn  btn-sm m-2 {{ \App\CommonHelpers::getMetaValue(auth()->user()->id, 'enable_long_multithread', 0) == 1 ? 'btn-primary' : 'btn-danger' }}">{{ \App\CommonHelpers::getMetaValue(auth()->user()->id, 'enable_long_multithread', 0) == 1 ? 'LONG is Enabled' : 'Long is Disabled' }}</a>
        ...
        <a href="{{ route('user.toggle-market') }}"
            class="btn  btn-sm m-2 {{ \App\CommonHelpers::getMetaValue(auth()->user()->id, 'enable_spot', 0) == 1 ? 'btn-info' : 'btn-warning' }}">{{ \App\CommonHelpers::getMetaValue(auth()->user()->id, 'enable_spot', 0) == 1 ? 'Spot Mode' : 'Future Mode' }}</a>
