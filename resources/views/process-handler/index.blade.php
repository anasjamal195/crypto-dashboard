...
@section('content')
    <div class="container">
        ...
        <a href="{{ route('user.toggle-position', 'LONG') }}"
            class="btn mb-2 float-right {{ \App\CommonHelpers::getSettingsValue('enable_long_multithread', 0) == 1 ? 'btn-success' : 'btn-danger' }}">{{ \App\CommonHelpers::getSettingsValue('enable_long_multithread', 0) == 1 ? 'LONG is Enabled' : 'LONG is Disabled' }}</a>

        <a href="{{ route('user.toggle-position', 'SHORT') }}"
            class="btn mb-2 float-right {{ \App\CommonHelpers::getSettingsValue('enable_short_multithread', 0) == 1 ? 'btn-success' : 'btn-danger' }}">{{ \App\CommonHelpers::getSettingsValue('enable_short_multithread', 0) == 1 ? 'SHORT is Enabled' : 'SHORT is Disabled' }}</a>
