...
@section('content')
<div class="container">
    <h1 class="text-center mb-4">Trade Statistics Spot</h1>
    ...
    <form method="GET" action="{{ route('live.trades.result', 'SPOT') }}">
        ...
        <div class="row mb-4">
            ...
            <div class="col-md-2 d-flex align-items-end">
                <a href="{{ route('live.trades.result', 'SPOT') }}?interval={{ request()->get('interval') }}"
        ...
        <div class="row mb-4">
            <div class="col-md-3">
                <a href="{{ route('live.trades.result', 'SPOT') }}?interval=1m&start_date={{ \Carbon\Carbon::now('Asia/Karachi')->subHour()->format('Y-m-d H:i:s') }}&end_date={{ \Carbon\Carbon::now('Asia/Karachi')->format('Y-m-d H:i:s') }}"
                    class="btn btn-primary btn-block">
                    Last 1 Hr
                </a>
            </div>
            <div class="col-md-3">
                <a href="{{ route('live.trades.result', 'SPOT') }}?interval=1m&start_date={{ \Carbon\Carbon::now('Asia/Karachi')->subDay()->format('Y-m-d H:i:s') }}&end_date={{ \Carbon\Carbon::now('Asia/Karachi')->format('Y-m-d H:i:s') }}"
                    class="btn btn-success btn-block">
                    Last 24 Hr
                </a>
            </div>
            <div class="col-md-3">
                <a href="{{ route('live.trades.result', 'SPOT') }}?interval=1m&start_date={{ \Carbon\Carbon::now('Asia/Karachi')->subWeek()->format('Y-m-d H:i:s') }}&end_date={{ \Carbon\Carbon::now('Asia/Karachi')->format('Y-m-d H:i:s') }}"
                    class="btn btn-warning btn-block">
                    Last Week
                </a>
            </div>
            <div class="col-md-3">
                <a href="{{ route('live.trades.result', 'SPOT') }}?interval=1m&start_date={{ \Carbon\Carbon::now('Asia/Karachi')->subMonth()->format('Y-m-d H:i:s') }}&end_date={{ \Carbon\Carbon::now('Asia/Karachi')->format('Y-m-d H:i:s') }}"
...
<div class="table-container">
    <table class="table dataTable">
        ...
        <tbody>
            ...
            $orderClose = DB::table('live_trades_spot_results')
            ->where('orderId', $order->pairId)
            ->first();

            $sqlTimestamp = $order->created_at;
            $orderCreated = Carbon::createFromFormat('Y-m-d H:i:s', $sqlTimestamp, 'Asia/Karachi');
            $differenceInMinutes = 0;
            // Check if $orderClose exists
            if (isset($orderClose)) {
            $orderCloseTime = Carbon::createFromFormat('Y-m-d H:i:s', $orderClose->created_at, 'Asia/Karachi');
            $differenceInMinutes = $orderCreated->diffInMinutes($orderCloseTime);
            } else {
            $now = Carbon::now('Asia/Karachi');
            $differenceInMinutes = $orderCreated->diffInMinutes($now);
            }
            ...
            <tr>
                ...
                <td>
                    @if (!$orderClose)
                    <a href="{{ route('live.trades.spot.close', $order->orderId ?? 0) }}"
                        class="btn btn-primary btn-sm" role="button">
                        <i class="fas fa-times"></i>
                    </a>
                    @else
                    @php
                    $sqlTimestampClose = $orderClose->created_at;
                    $dateClose = Carbon::createFromFormat('Y-m-d H:i:s', $sqlTimestampClose, 'Asia/Karachi');



                    $dateClose->setTimezone('GMT');
                    @endphp

                    {{ $dateClose->setTimezone('Asia/Karachi')->format('H:i:s') }}<br>{{ $dateClose->format('M d, Y') }}
