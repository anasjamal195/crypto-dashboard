...
@section('content')
   @php
        ...
        $users = \App\User::where('role','trader')->get();

        $wallets = [];

        foreach($users as $user){
         $wallets[] =[
            'futureWallet' => BinanceApiService::fetchFutureWalletDetails($user->id),
            'spotWallet' => BinanceApiService::fetchSpotWalletDetails($user->id),
        ];
        }
    @endphp
    <style>
