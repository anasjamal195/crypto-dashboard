                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <select name="showTimeline" id="showTimeline" class="form-control select2">
                                    ...
                                    <option value="show" {{ request('showTimeline') == 'show' ? 'selected' : '' }}>Shown
                                    </option>
...
<script>
    ...
    function createTimelineChart(data) {
        ...
        const datasets = data.map((item, index) => {
            return {
                label: item.symbol,
                data: [{
                    x: [new Date(item.startTime.replace(' ', 'T')), new Date(item.endTime.replace(' ', 'T'))],
                    y: item.symbol
                }],
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                ...
                                const duration = (new Date(datapoint.x[1]) - new Date(datapoint.x[0])) / 1000;
    ...
    function updateTimelineCursor(chart, event, cursorInfoId, cursorLineId, data) {
        ...
        if (x >= chartArea.left && x <= chartArea.right) {
            ...
            cursorInfo.innerHTML = `<strong>Time:</strong> ${cursorTimeFormatted} | <strong>Active Trades:</strong> ${intersectionCount}`;
    ...
    // Example of how you would fetch and load data from an API
    // Uncomment this block to use actual API data
    /*
