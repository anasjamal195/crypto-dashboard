{{-- resources/views/dashboard/wallet.blade.php --}}
                    ...
                    <div id="futuresWalletStats" class="row mb-4">
                        @if($futureWallet)
                            <div class="col-lg-3 col-md-6 mb-3">
                                <div class="stat-card">
                                    <div class="stat-value">${{ number_format($futureWallet['wallet_balance'], 2) }}</div>
                                    <div class="stat-label">Total Wallet Balance</div>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-6 mb-3">
                                <div class="stat-card">
                                    <div class="stat-value {{ $futureWallet['unrealized_profit'] >= 0 ? 'profit-positive' : 'profit-negative' }}">
                            ...
                            <div class="col-lg-3 col-md-6 mb-3">
                                <div class="stat-card">
                                    <div class="stat-value">${{ number_format($futureWallet['margin_balance'], 2) }}</div>
                                    <div class="stat-label">Margin Balance</div>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-6 mb-3">
                                <div class="stat-card">
                                    <div class="stat-value">${{ number_format($futureWallet['available_balance'], 2) }}</div>
                                ...
                                @if($futureWallet && count($futureWallet['positions']) > 0)
                                    @foreach($futureWallet['positions'] as $position)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="position-badge {{ floatval($position['positionAmt']) > 0 ? 'position-long' : 'position-short' }}">
                                            ...
                                            <td class="{{ floatval($position['unRealizedProfit']) >= 0 ? 'profit-positive' : 'profit-negative' }}">
                                                {{ floatval($position['unRealizedProfit']) >= 0 ? '+' : '' }}${{ number_format(floatval($position['unRealizedProfit']), 2) }}
                                            </td>
                                            <td>{{ $position['leverage'] }}x</td>
                                            <td>
                                                <span class="badge badge-{{ $position['marginType'] === 'cross' ? 'info' : 'warning' }}">
                ...
                <div class="card-body">
                    <div class="row mb-4">
                        @if($spotWallet)
                            ...
                            <div class="col-lg-3 col-md-6 mb-3">
                                <div class="stat-card">
                                    <div class="stat-value">${{ number_format($totalSpotBalance + $totalSpotLocked, 2) }}</div>
                                    <div class="stat-label">Total Balance</div>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-6 mb-3">
                                <div class="stat-card">
                                    <div class="stat-value">{{ isset($spotWallet['open_orders']) ? count($spotWallet['open_orders']) : 0 }}</div>
                                ...
                                @if($spotWallet && isset($spotWallet['total_assets']) && count($spotWallet['total_assets']) > 0)
                                    @foreach($spotWallet['total_assets'] as $asset)
                                        <tr>
                                            ...
                                            <td>{{ number_format(floatval($asset['free']) + floatval($asset['locked']), 8) }}</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
                                ...
                                <tbody>
                                    @foreach($spotWallet['open_orders'] as $order)
                                        <tr>
                                            ...
                                            <td>
                                                <span class="position-badge {{ $order['side'] === 'BUY' ? 'position-long' : 'position-short' }}">
                                                    {{ $order['side'] }}
                                                </span>
                                            </td>
                                            <td>{{ $order['type'] }}</td>
...
function refreshWalletData() {
    ...
    .then(data => {
        if (data.success) {
            ...
            alert('Failed to refresh wallet data: ' + (data.message || 'Unknown error'));
        }
    })
    .catch(error => {
        console.error('Error:', error);
        alert('An error occurred while refreshing wallet data');
    })
    .finally(() => {
