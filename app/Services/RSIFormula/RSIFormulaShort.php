...
namespace App\Services\RSIFormula;
...
class RSIFormulaShort
{
    ...
    public static function performLiveTrades($market, $account = null)
    {
        ...
        foreach ($tradeHandler as $tradeInstance)
            try {
                ...
                Log::info('RSIFormulaShort: Current Trade');
                Log::info('RSIFormulaShort: Coin: ' . $symbol);
                Log::info('RSIFormulaShort: Account: ' . $trade_acc);
                Log::info('RSIFormulaShort: Invested: ' . $buy_coin_price . ' $');

                CommonHelpers::checkLosses($symbol, 'SHORT', $tradeInstance->tradeAccount, 'RSI Formula');
                ...
                Log::info('RSIFormulaShort: Closing time gap: ' .  (now()->timestamp - $candleData[count($candleData) - 1]['binance_timestamp'] / 1000) . ' seconds');
                ...
                if (isset($open_order['is_open']) && $open_order['is_open']) {
                    ...
                    $rsiCondition = $secondLastCandle['rsi6'] <= 73;
                    ...
                    if ($supportResistanceBand && $candleTrend && $finalThresholdCondition && $rsiCondition) {
                        ...
                        if ($lastOrderClose) {
                            ...
                            if ($timeDiff < 20) {
                                Log::info('RSIFormulaShort: Skipped due to last order close time: ' . $symbol);
                                continue;
                            }
                        }
                        Log::info('RSIFormulaShort: Conditions Staisfied, opening now : ' . $symbol);
    ...
    {
        Log::info('RSIFormulaShort: Open order found for ' . $buy_order['symbol']);
        ...
        if ($targetProfit <= 1) {
            ...
            Log::info('RSIFormulaShort: Current profit ' . $currentProfit);
            ...
            if ($currentCandle['close'] > $stopLoss) {
                ...
                if ($currentProfit < 0) {
                    Log::info('RSIFormulaShort: Placing new order to recover from loss of previous order: ' . $tradeInstance->symbol);
                    BinanceApiService::openMarketPositionLiveTrader($tradeInstance->symbol, $tradeInstance->buyPrice, $tradeInstance->position === 'LONG' ? 'SELL' : 'BUY', $tradeInstance->leverage, $tradeInstance->tradeAccount, 'RSI Formula');
                }
