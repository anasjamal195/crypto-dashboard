...
class RSIFormulaLong
{
    ...
    public static function performLiveTrades($market, $account = null)
    {
        ...
        foreach ($tradeHandler as $tradeInstance)
            try {
                ...
                Log::info('RSIFormulaLong: Current Trade');
                Log::info('RSIFormulaLong: Coin: ' . $symbol);
                Log::info('RSIFormulaLong: Account: ' . $trade_acc);
                Log::info('RSIFormulaLong: Invested: ' . $buy_coin_price . ' $');
                ...
                Log::info('RSIFormulaLong: Closing time gap: ' .  (now()->timestamp - $candleData[count($candleData) - 1]['binance_timestamp'] / 1000) . ' seconds');
                CommonHelpers::checkLosses($symbol, 'LONG', $tradeInstance->tradeAccount, 'RSI Formula');
                ...
                if (isset($open_order['is_open']) && $open_order['is_open']) {
                    ...
                    $supportResistanceBand = $currentCandle['close'] >= ($supportResistance[7]['support'] * (1 - 0.3 / 100)) && $currentCandle['close'] <= ($supportResistance[7]['support'] * (1 + 0.3 / 100));
                    ...
                    $stochCondition = $currentCandle['stoch_d'] <= 8;
                ...
                Log::error('RSIFormulaLong: Error - ' . $e->getMessage());
    ...
    private static function manageOpenOrder($tradeInstance,  $buy_order, $supportResistance, $isCandleClosing): void
    {
        ...
        Log::info('RSIFormulaLong: Open order found for ' . $buy_order['symbol']);
        ...
        if ($targetProfit <= 1) {
            ...
            Log::info('RSIFormulaLong: Current profit ' . $currentProfit);
