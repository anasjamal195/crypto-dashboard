            ...
            foreach ($dynamicTrades as $trade) {
                ...
                if ($trade->side == 'BUY') {
                    if ($currentPrice < $trade->priceLock) {
                        ...
                        Log::info("Executing BUY order for trade {$trade->id} due to price increase beyond buffer.");
                        if (!$trade->openOrderId) {
                            BinanceApiService::openMarketPosition($trade->symbol, $trade->amount, $trade->position, $trade->leverage ?? 0, $trade->tradeAccount);
                        } else {
                            BinanceApiService::closeMarketPosition($trade->openOrderId);
                        }
                        DB::table('dynamic_trade_handler')->where('id', $trade->id)->update([
                            'status' => 'FILLED',
                            'isActive' => 0,
