...
namespace App\Services\VSAFormula;
...
class VSAFormulaShort
{
    ...
    public static function performLiveTrades($market, $account = null)
    {
        ...
        foreach ($tradeHandler as $tradeInstance)
            try {
                ...
                Log::info('VSAFormulaShort: Current Trade');
                Log::info('VSAFormulaShort: Coin: ' . $symbol);
                Log::info('VSAFormulaShort: Account: ' . $trade_acc);
                Log::info('VSAFormulaShort: Invested: ' . $buy_coin_price . ' $');
                ...
                CommonHelpers::checkLosses($symbol, 'SHORT', $tradeInstance->tradeAccount, 'VSA Formula');

                Log::info('VSAFormulaShort: Closing time gap: ' .  (now()->timestamp - $candleData[count($candleData) - 1]['binance_timestamp'] / 1000) . ' seconds');
                ...
                if (isset($open_order['is_open']) && $open_order['is_open']) {
                    ...
                    Log::info('VSAFormulaShort: Support: ' . $supportResistanceContition);
                    Log::info('VSAFormulaShort: MA Condition: ' . $maCondition);
                    Log::info('VSAFormulaShort: MA MACandleDistance: ' . $maCandleDistance);
                    ...
                    if ($supportResistanceContition && $maCondition && $proceedCondition) {
                        ...
                        if ($lastOrderClose) {
                            ...
                            if ($timeDiff < 20) {
                                Log::info('VSAFormulaShort: Skipped due to last order close time: ' . $symbol);
                                continue;
                            }
                        }
                        Log::info('VSAFormulaShort: Conditions Staisfied, opening now : ' . $symbol);

                        BinanceApiService::openMarketPositionLiveTrader($tradeInstance->symbol, $tradeInstance->buyPrice, $tradeInstance->position === 'LONG' ? 'BUY' : 'SELL', $tradeInstance->leverage, $tradeInstance->tradeAccount, 'VSA Formula');
                    }
                ...
                Log::error('VSAFormulaShort: Error - ' . $e->getMessage());
    ...
    {
        Log::info('VSAFormulaShort: Open order found for ' . $buy_order['symbol']);
        ...
        if ($targetProfit <= 1) {
            ...
            Log::info('VSAFormulaShort: Current profit ' . $currentProfit);
            ...
            if ($currentCandle['close'] > $stopLoss) {
                ...
                if ($currentProfit < 0) {
                    Log::info('VSAFormulaShort: Placing new order to recover from loss of previous order: ' . $tradeInstance->symbol);
                    BinanceApiService::openMarketPositionLiveTrader($tradeInstance->symbol, $tradeInstance->buyPrice, $tradeInstance->position === 'LONG' ? 'SELL' : 'BUY', $tradeInstance->leverage, $tradeInstance->tradeAccount, 'VSA Formula');
                }
