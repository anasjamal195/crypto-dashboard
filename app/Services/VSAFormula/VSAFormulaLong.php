...
class VSAFormulaLong
{
    ...
    public static function performLiveTrades($market, $account = null)
    {
        ...
        foreach ($tradeHandler as $tradeInstance)
            try {
                ...
                Log::info('VSAFormulaLong: Current Trade');
                Log::info('VSAFormulaLong: Coin: ' . $symbol);
                Log::info('VSAFormulaLong: Account: ' . $trade_acc);
                Log::info('VSAFormulaLong: Invested: ' . $buy_coin_price . ' $');

                CommonHelpers::checkLosses($symbol, 'LONG', $tradeInstance->tradeAccount, 'VSA Formula');
                ...
                Log::info('VSAFormulaLong: Closing time gap: ' .  (now()->timestamp - $candleData[count($candleData) - 1]['binance_timestamp'] / 1000) . ' seconds');
                ...
                if (isset($open_order['is_open']) && $open_order['is_open']) {
                    ...
                    Log::info('VSAFormulaLong: MA Condition: ' . $maCondition);
                    Log::info('VSAFormulaLong: MA MACandleDistance: ' . $maCandleDistance);
                    ...
                    if ($supportResistanceContition && $maCondition && $proceedCondition) {
                        ...
                        if ($lastOrderClose) {
                            ...
                            if ($timeDiff < 20) {
                                Log::info('VSAFormulaLong: Skipped due to last order close time: ' . $symbol);
                                continue;
                            }
                        }
                ...
                Log::error('VSAFormulaLong: Error - ' . $e->getMessage());
    ...
    private static function manageOpenOrder($tradeInstance,  $buy_order, $supportResistance, $isCandleClosing): void
    {
        ...
        Log::info('VSAFormulaLong: Open order found for ' . $buy_order['symbol']);
        ...
        if ($targetProfit <= 1) {
            ...
            Log::info('VSAFormulaLong: Current profit ' . $currentProfit);
            ...
            if ($currentCandle['close'] < $stopLoss) {
                ...
                if ($currentProfit < 0) {
                    Log::info('VSAFormulaLong: Placing new order to recover from loss of previous order: ' . $tradeInstance->symbol);
