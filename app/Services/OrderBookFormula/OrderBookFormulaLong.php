...
class OrderBookFormulaLong
{
    ...
    public static function performLiveTrades($market, $account = null)
    {
        ...
        try {
            $workerLimit = 10;
            $interval = CommonHelpers::getMetaValue($account,'live_trade_worker_interval_future','1m');
            $openSymbols = DB::table('live_trades_future_results')
                ->where('trade_acc', $account)
                ->where('trade_status', 'open')
                ->pluck('symbol');

            $uniqueSymbols = DB::table('trade_handler')
                ->where('tradeAccount', $account)
                ->where('market', $market)
                ->where('isWorkerDispatched', false)
                ->where('position', 'LONG')
